...
class Home extends Component{
    ...
    componentDidMount(){
        axios.get('http://localhost:8080/DailyIncidentTrackerProject/incident/getAllIncidents').then(response =>{
            this.setState({
                imrs : response.data.Incident
            })
        })
        // this.setState({
        //     imrs : openIMRs
    ...
    render(){
        ...
        const currentPage = this.state.currentPage==='servers'?(
            ...
            < ReportWithChartJS imrs={this.state.imrs}/>
        ):this.state.currentPage==='charts'?(
            <Report imrs={this.state.imrs}/>
